...
public abstract class RadiusServer {
	LogExtHelper logExt = null;
	public RadiusServer() {
	}
	
	public RadiusServer(ILog log) {
		this.logExt = new LogExtHelper(log);
	}
	/**
	...
	throws RadiusException {
		String plaintext = getUserPassword(accessRequest.getUserName());//getUserPassword(accessRequest.getUserName());
		...
		if (listenAuth) {
			new Thread() {
				public void run() {
					...
					try {
						String message = "starting RadiusAuthListener on port " + getAuthPort();
						logger.info(message);
						logExt.Info(message);
						listenAuth();
						message = "RadiusAuthListener is being terminated";
						logger.info(message);
						logExt.Info(message);
					} catch(Exception e) {
						e.printStackTrace();
